...
$(function() {
	...
	trackingSensorChart= new Highcharts.Chart({
		chart : {
			renderTo:"trackingSensorChartContainer",  //차트는 여기 나타내게 할것이고
			type: "spline",   //라인그래프형식
			//데이터를 누가 제공해줄것이냐
			events: {
				load: requesttrackingSensorData //함수이름(차트객체가 생성되고 나서 해당 함수를 실행시키는 코드)
			}
		},
		colors: ['cyan'],
		title:{
			text:"trackingSensorChart(트레킹센서)"
		},
		xAxis:{
...
function requesttrackingSensorData(){
	...
	ws.onmessage = function(event){
		...
		series.addPoint([data.time,data.tracking],true,shift); //[x축,y축]
	};
}
